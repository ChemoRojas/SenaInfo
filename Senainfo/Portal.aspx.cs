...
public partial class Portal : System.Web.UI.Page
{
    ...
    private int codtrab = 0;
    private string vigencia = String.Empty;
    private int contrasena = 0;

    private string tokenSSO = String.Empty;
    private string mensaje = String.Empty;

    public System.Data.DataTable dt = null;

    private string dns_from = (string)ConfigurationManager.AppSettings["dns_from"].ToString();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                tokenSSO = window.Base64Decode(Request.QueryString["t"]).ToString();
                ingreso();
            }
            else
            {
                mensaje = window.Base64Encode("Debe ingresar su usuario y contraseña");
                Response.Redirect(dns_from + "senainfo.cl?mensaje=" + mensaje);
            }
        }
    }
    ...
    protected void ingreso()
    {
        ...
        DataSet arr = ls.ObtenerTokensUsuarioSSO(tokenSSO);
        Session["tokens"] = arr;
        ...
        if (window.existetoken("3E85C221-1603-4D99-AA31-9EFD971F7387"))
        {
            ...
            Session["mm"] = gn.getMenu_x_Token(tokenSSO);

            //bool aacc = ls.ActualizarTokenSSO(tokenSSO);
            DataSet dstoken = ls.ActualizarTokenSSONuevo(tokenSSO);

            Session["TokenAux"] = dstoken.Tables[0].Rows[0].ItemArray[0].ToString();//AB:fix= .ItemArray[0]
    ...
    protected void datosUsuario()
    {
        ...
        con.Autenticar();
        dt = con.TraerDataTable("obtenerDatosUsuario", tokenSSO);

        DataRow row = dt.Rows[0];

        idusuario = int.Parse(row[0].ToString());
        Session["IdUsuario"] = idusuario;
        codtrab = int.Parse(row[1].ToString());
        Session["contrasena"] = int.Parse(row[2].ToString());
        contrasena = int.Parse(row[2].ToString());
        vigencia = row[3].ToString();

        string myString = (string)(row[4] + " " + row[5]).ToLower();
        TextInfo myTI = new CultureInfo("es-CL", false).TextInfo;
        nombreUsuario = myTI.ToTitleCase(myString);
        Session["Usuario"] = nombreUsuario;

        Session["UserName"] = row[7].ToString();

        Session["CodRegionUsuario"] = int.Parse(row[6].ToString());
    ...
    protected void registrosAdm()
    {
        //Graba fechaultimoingreso
        Conexiones con = new Conexiones();
        con.Autenticar();
        int salida = (int)con.TraerValorEscalar("SP_Usuarios_FechaUltimoIngreso", idusuario);
        ...
        */
        ...
        int registroIngreso = (int)con.TraerValorEscalar("PA_LogUsuarios", idusuario, NombreSistema, navegador.Browser.ToString(), navegador.Version.ToString(), clientIp.ToString());
        con.CerrarConexion();
    }

    protected void cargaParametricas()
    {
    ...
    #region private string get_indvigencia(string username)
    //private string get_indvigencia(string username)
    //{
    //    DbDataReader datareader = null;
    //    Conexiones con = new Conexiones();
    //    //con.ejecutar(Resources.Procedures.GetVigencia_Usuario + "'" + username + "'", out datareader);
    ...
    #region public int getIdUsuario(String user)
    //public int getIdUsuario(String user)
    //{
    //    DbDataReader datareader = null;

    //    Conexiones con = new Conexiones();
    //    //con.ejecutar(Resources.Procedures.GetVigencia_Usuario + "'" + user + "'", out datareader);
